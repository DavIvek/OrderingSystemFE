<template>
  <form @submit.prevent="submitForm">
    ...
    <div v-else>
      ...
      <div class="form-control" :class="{ invalid: !typeOfUser.isValid }">
        <h3>Tip</h3>
        <div>
          <input
            type="radio"
            id="Liječnik"
            value="Doctor"
            v-model="typeOfUser.val"
            @blur="clearValidity('typeOfUser')"
          />
          <label for="Liječnik" class="ignore">Liječnik</label>
        </div>
        <div>
          <input
            type="radio"
            id="Medicinska sestra"
            value="Nurse"
            v-model="typeOfUser.val"
            @blur="clearValidity('typeOfUser')"
          />
          <label for="Medicinska sestra" class="ignore"
            >Medicinska sestra</label
          >
        </div>
      ...
      <base-button>Kreiraj!</base-button>
    </div>
  </form>
</template>
...
export default {
  ...
  methods: {
    ...
    validateForm() {
      ...
      if (
        this.OIB.val === "" ||
        (this.OIB.val.length !== 11 && !Number.isInteger(this.OIB.val))
      ) {
      ...
      if (
        this.email.val === "" ||
        !this.email.val.match(/^[a-zA-Z0-9+_.-]+@[a-zA-Z0-9.-]+.+[a-zA-Z]+$/)
      ) {
      ...
      if (this.typeOfUser.val.length === 0) {
        this.typeOfUser.isValid = false;
        this.formIsValid = false;
      }
    },
    async submitForm() {
      ...
      const formData = {
        ...
        role: {name:this.typeOfUser.val.toUpperCase(), description:this.typeOfUser.val}
      };
      ...
      let path=""
      if(this.typeOfUser.val === "Doctor"){
        path = process.env.VUE_APP_BACKEND_URL + "/auth/signup/doctor"
      }
      else{
        path = process.env.VUE_APP_BACKEND_URL +"/auth/signup/nurse"
      }
        let res = await fetch(
          path,
          requestOptions
