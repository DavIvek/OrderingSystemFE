<template>
  ...
  <div v-if="calendarSelected == 'cal'" style=" padding: 2%;"> 
    <vue-cal class="vuecal--blue-theme" :disable-views="['years', 'year', 'month']" :events="events" :min-date="minDate" :max-date="maxDate" :time-cell-height="43" :time-from="6 * 60" :time-to="23*60" :time-step="30" locale="hr"/>    
  </div>
  <transition>
              ...
              <div v-if="dayFrom <= dayTo">
                ...
                <div class="schedule">
                  <div class="setting" id="scheduleSetting">
                    ...
                    <div>
                      <h2>Odaberi dostupnost usluga:</h2>
                      <div v-for="(service,index) in services" :key = "index">
                        <h3> {{service.name}} </h3>
                        <div class="option">
                          <label for="evenStart">Od: </label>
                          <input v-model="even.serviceModelFrom[index]" type="time" id="evenStart" name="evenStart">
                          <label for="evenEnd"> Do: </label>
                          <input v-model="even.serviceModelTo[index]" type="time" id="evenEnd" name="evenEnd">
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="setting" id="scheduleSetting">
                    ...
                    <div>
                      ...
                      <div v-for="(service,index) in services" :key = "index">
                        ...
                        <div class="option">
                          <label for="oddStart">Od: </label>
                          <input v-model="odd.serviceModelFrom[index]" type="time" id="oddStart" name="oddStart">
                          <label for="oddEnd"> Do: </label>
                          <input v-model="odd.serviceModelTo[index]" type="time" id="oddEnd" name="oddEnd">
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div>
                  <base-button v-if="evenTime != '' && oddTime != '' &&  evenpauseStart != '' && oddpauseStart != '' && evenpauseLength != 'pick' && oddpauseLength != 'pick'
                  && even.serviceModelFrom.length == services.length && even.serviceModelTo.length == services.length && odd.serviceModelFrom.length == services.length && odd.serviceModelTo.length == services.length 
                  && even.serviceModelFrom.every((val, i, arr) => val <= even.serviceModelTo[i]) && odd.serviceModelFrom.every((val, i, arr) => val <= odd.serviceModelTo[i])"
  ...
  methods: {
    confirmAppeared: async function() {
      ...
      setApps: function(data) {
        this.events = data.map(app => {
          return {
            ...
            title: app.patient != null ? app.patient.firstname + " " + app.patient.lastname +'  '+ app.medicalService.name: "Prazan termin"
          }
        })
        this.allChecksDelay = data.filter(appointment => new Date(appointment.start).toDateString() == new Date().toDateString() && appointment.start > new Date().getTime() && appointment.patient != null)
        this.allChecks = data.filter(appointment => new Date(appointment.start).toDateString() == new Date().toDateString() && appointment.start < new Date().getTime() && !appointment.patientAttended && appointment.patient != null)
      },
      ...
      checkSchedule: function() {
        ...
        if (!this.even.serviceModelFrom.every((val, i) => this.addMinutes(val, 15) <= this.even.serviceModelTo[i]) || !this.odd.serviceModelFrom.every((val, i) => this.addMinutes(val, 15) <= this.odd.serviceModelTo[i])){
          return false
        }
        let tempeven = this.even.serviceModelFrom.concat(this.even.serviceModelTo)
        tempeven.sort()
        let tempodd = this.odd.serviceModelFrom.concat(this.odd.serviceModelTo)
        tempodd.sort()
        for(let i = 0; i < this.services.length; i++){
          if (tempeven.indexOf(this.even.serviceModelFrom[i]) != tempeven.indexOf(this.even.serviceModelTo[i]) - 1){
            return false
          }
          if (tempodd.indexOf(this.odd.serviceModelFrom[i]) != tempodd.indexOf(this.odd.serviceModelTo[i]) - 1){
            return false
          }
          let tefr = this.even.serviceModelFrom[i].split(":").map(o => parseInt(o))
          let teto = this.even.serviceModelTo[i].split(":").map(o => parseInt(o))
          if (new Date().setHours(tefr[0],tefr[1],0,0) <= new Date().setHours(timestarte[0],timestarte[1],0,0) || new Date().setHours(teto[0],teto[1],0,0) >= new Date().setHours(timeende[0],timeende[1],0,0)){
            return false
          }
          let tofr = this.odd.serviceModelFrom[i].split(":").map(o => parseInt(o))
          let toto = this.odd.serviceModelTo[i].split(":").map(o => parseInt(o))
          if (new Date().setHours(tofr[0],tofr[1],0,0) <= new Date().setHours(timestarto[0],timestarto[1],0,0) || new Date().setHours(toto[0],toto[1],0,0) >= new Date().setHours(timeendo[0],timeendo[1],0,0)){
            return false
          }
        }
        return true
      ...
      getOdd: function(date) {
        ...
        for(let i = 0; i < this.services.length; i++){
          let tofr = this.odd.serviceModelFrom[i].split(":").map(o => parseInt(o))
          let toto = this.odd.serviceModelTo[i].split(":").map(o => parseInt(o))
          for (let j = new Date(date).setHours(tofr[0],tofr[1],0,0); j <= new Date(date).setHours(toto[0],toto[1],0,0)-15*60000; j +=15*60000){
            termini.push({start: j, end: j+15*60000, medicalServiceId: this.services[i].id})
          }
        }
        return termini
      },
      getEven: function(date) {
        ...
        for(let i = 0; i < this.services.length; i++){
          let tefr = this.even.serviceModelFrom[i].split(":").map(o => parseInt(o))
          let teto = this.even.serviceModelTo[i].split(":").map(o => parseInt(o))
          for (let j = new Date(date).setHours(tefr[0],tefr[1],0,0); j <= new Date(date).setHours(teto[0],teto[1],0,0)-15*60000; j +=15*60000){
            termini.push({start: j, end: j+15*60000, medicalServiceId: this.services[i].id})
          }
        }
        return termini
    ...
    async fetchData() {
      let loginData = JSON.parse(localStorage.getItem("orderingSystemData"));
        ...
        const res1 = await fetch(process.env.VUE_APP_BACKEND_URL + "/services", requestOptions)
        ...
        for(let i = 0; i < this.services.length; i++){
          this.indexToId[i] = this.services[i].id
        }
